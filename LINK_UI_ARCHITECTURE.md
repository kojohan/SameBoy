...
The first direct-IP implementation of this contract, its physical two-PC four-mode regression and its bidirectional reconnect lifecycle are complete. The SDL frontend has explicit session modes, a compact `Link` menu, persistent independent P1/P2 keyboard/controller mappings, menu-driven Local Link/Host/Join/Disconnect and retained CLI regression paths. Protocol v8 gives Remote Client visible pre-frame connection and pairing status. `Host Remote Link…` detects the LAN endpoint, creates a fresh internal key for that host lifetime, copies an endpoint/session invite and starts hosting. `Join Remote Link…` imports the invite, receives the key automatically as the first client and confirms it; later clients are rejected until Host ends. No key control appears in the UI. Port, Session ID and manual address remain under `Remote Settings… > Network Settings…`. Opaque server-backed room codes and Direct/Relay labeling remain future work.
...
`Player 1 connected` on the client gameplay image. These are transient local
presentation notices rather than text transported inside the video stream.
